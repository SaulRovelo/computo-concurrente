...
int peatones_esperando = 0; //Agregamos una nueva variable 
...
void * peaton(void * arg){
	...
	//Bloqueamos el acceso al contador de los peatones 
	
	pthread_mutex_lock(&mutex);
    peatones_esperando++; // Cuando se llame a la funcion aumentaremos 1 al contador, para 
	//saber cuantos peatones estan esperando, antes de bloquearse en la espera del semaforo.
    pthread_mutex_unlock(&mutex);
	...
	// Despertamos a cada peaton uno por uno con ayuda del contador que vamos a simular 
	//Una fila de espera, cada que se llame al peaton vamos a sumar 1 al contador 
	//y cuando lleguemos a este punto le restamos uno y despertamos un peaton con el signal y asi
	//hasta acabar con todos los peatones
    //peatones_esperando--;
    if (peatones_esperando > 0) {
        pthread_cond_signal(&condicion); // Despierta el peaton. 
    }
	pthread_mutex_unlock(&mutex);
	pthread_exit(NULL);
}
...
int main(int argc, char * argv[]){
	...
	for(int i=0;i<num_peatones;++i){
		int *id = (int *) malloc(sizeof(int));//cada ranita tiene su identificador "unico"
		* id = i+1;
